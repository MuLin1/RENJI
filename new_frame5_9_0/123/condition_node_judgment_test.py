...
try:
    ...
    module_candidates = [
        ...
        os.path.join(project_root, PAPER_VERSION_DIR)
    ]
    paper_version_path = next((path for path in module_candidates if os.path.exists(path)), None)
    if not paper_version_path:
        raise FileNotFoundError(f"找不到模块目录: {module_candidates}")
...
def load_label_to_id_data():
    ...
    try:
        label_path = os.path.join(script_dir, LABEL_TO_ID_FILE)
        ...
        print(f"✗ 找不到标注文件: {os.path.join(script_dir, LABEL_TO_ID_FILE)}")
        sys.exit(1)
...
def load_test_data() -> Dict[str, Dict]:
    ...
    try:
        test_data = {}
        test_data_path = os.path.join(script_dir, TEST_DATA_FILE)
        ...
        print(f"✗ 找不到测试数据文件: {os.path.join(script_dir, TEST_DATA_FILE)}")
        sys.exit(1)
...
def run_evaluation():
    ...
    output_path = os.path.join(script_dir, EVALUATION_OUTPUT)
...
if __name__ == "__main__":
    label_path = os.path.join(script_dir, LABEL_TO_ID_FILE)
    test_data_path = os.path.join(script_dir, TEST_DATA_FILE)
    if not all(os.path.exists(f) for f in [label_path, test_data_path]):
        print("✗ 必需的测试文件不存在，请检查路径配置。")
        if not os.path.exists(label_path): print(f"  - 找不到: {label_path}")
        if not os.path.exists(test_data_path): print(f"  - 找不到: {test_data_path}")
        sys.exit(1)
