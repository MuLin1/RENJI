...
def split_dialogue_into_rounds(dialogue: List[Any]) -> Tuple[List[Dict[str, str]], List[str]]:
    ...
    for turn in dialogue:
        if isinstance(turn, dict):
            ...
            if role == "patient":
                ...
                continue
            if role == "doctor":
                all_doctor_turns.append(content)
                if pending_patient:
...
def annotate_single_sample(sample: Dict[str, Any], sample_index: int, total_count: int) -> Dict[str, Any]:
    ...
    pid = extract_pid(sample, sample_index)
    dialogue = extract_dialogue(sample)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(concurrency, total_samples)) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            try:
                ...
                result = {
                    "sample_index": idx,
                    "pid": extract_pid(samples[idx], idx),
                    "status": "error",
                    "error_message": str(e),
